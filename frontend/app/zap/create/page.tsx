...
function useAvailableActionsAndTriggers() {
    const [availableActions,selectedAvailableActions] = useState([]);
    const [availableTriggers,selectedAvailableTriggers] = useState([]);
    useEffect(() =>{
        axios.get(`${BACKEND_URL}/api/v1/trigger/available`)
        .then(x =>  selectedAvailableTriggers(x.data.availableTriggers))
        axios.get(`${BACKEND_URL}/api/v1/action/available`)
        .then(x =>  selectedAvailableActions(x.data.availableActions))
    ...
    <div className="relative p-4 w-full max-w-2xl max-h-full">
        <div className="relative bg-white rounded-lg shadow">
            <div className="flex items-center justify-between border-b border-default pb-4 md:pb-5">
                <div className="text-xl pl-4 pt-4">
                    Select {index ===1 ? "Trigger":"Action"}
                </div>
                <button onClick={() =>{
                    onSelect(null);
                }} type="button" className="text-body bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="default-modal">
            ...
            <div className="p-4 md:p-5 space-y-4">
                {availableItems.map(({id,name,image}) =>{
                    ...
                    }} className="flex border p-4 cursor-pointer hover-slate-100">
